...
# 引入 utils 中的所有内容
from .utils import * # 显式引入必要的函数，确保它们能被正确识别
from .utils import (
    ChatGPT_API,
    ChatGPT_API_async,                 # 建议加上异步API支持
    add_node_text,                     # 修复之前的 NameError
    generate_summaries_for_structure,  # 修复刚才的 NameError (注意这里必须有逗号)
    add_preface_if_needed,
    convert_page_to_int,
    clean_page_numbers                 # 加上这个来修复 'str' and 'int' 错误
)
# page_index.py 头部
from .utils import (
    ChatGPT_API,
    ...
    convert_page_to_int,
    clean_page_numbers,
    remove_structure_text,  # <--- 确保添加这一行，或者直接保留 import *
    extract_json,
    count_tokens,
    get_page_tokens,
    write_node_id,
    ...
    ConfigLoader,
    get_pdf_name,
    convert_physical_index_to_int # 确保这些都被引入
)
################### check title in page #########################################################
async def check_title_appearance(item, page_list, start_index=1, model=None):    
    ...
    try:
        # 强制转换为 int，防止 'str' - 'int' 错误
        page_number = int(item['physical_index'])
    except (ValueError, TypeError):
        return {'list_index': item.get('list_index'), 'answer': 'no', 'title': title, 'page_number': None}
    # --- 修复结束 ---
    
    # 增加边界检查，防止 IndexError
